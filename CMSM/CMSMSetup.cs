...
namespace CMSM
{
	...
	/// </summary>
	[RunInstaller(true)]
	public class CMSMSetup : System.Configuration.Install.Installer
	{
		/// <summary>
		/// 必需的设计器变量。
		/// </summary>
		...
		public override void Install(IDictionary stateSaver)
		{
			...
			try
			{
				System.IO.FileInfo fileInfo = new System.IO.FileInfo(this.Context.Parameters["targetdir"]+@"\CMSM.exe.config");
				if (!fileInfo.Exists)
				{
				...
				foreach (System.Xml.XmlNode node in xmlDocument["configuration"]["appSettings"])
				{
					if (node.Name == "add")
					{
						if (node.Attributes.GetNamedItem("key").Value == "DBAMSCMCenter")
						{
				...
				if (!bFoundIt)
				{
					throw new InstallException("CMSM.exe.Config 文件没有包含连接字符串设置");
				}
